...
nlohmann::json handle_result(
  ...
  // Guard against missing connection entry
  if (data_msg == nullptr) {
    LOG(ERROR) << "handle_result: data_msg is null, hdl not found in data_map";
    return jsonresult;
  }
  ...
  if (tmp_online_msg != "") {
    ...
    if (!data_msg->is_sentence_started) {
      ...
      jsonresult["slice_type"] =
          0; // 0：一段话开始识别; 1：一段话识别中; 2：一段话识别结束
    ...
    if ((jsonresult["is_speaking"] == false ||
      ...
      try {
        ...
        msg_data->strand_->post(
            std::bind(&WebSocketServer::do_decoder, this, data_map,
                      ...
                      std::move(true), msg_data->msg["wav_name"],
  ...
  case websocketpp::frame::opcode::binary: {
    ...
    if (isonline) {
      ...
      if (sample_data_p->size() > setpsize) {
        ...
        std::vector<char> subvector = {sample_data_p->begin(),
                                       sample_data_p->begin() +
                                           chunksize * setpsize};
        ...
        try {
          ...
          if (msg_data->msg["is_eof"] != true &&
            ...
            msg_data->strand_->post(std::bind(
                &WebSocketServer::do_decoder, this, data_map,
                std::move(subvector), std::move(hdl), std::ref(msg_data->msg),
                std::ref(*(punc_cache_p.get())), std::move(hotwords_embedding_),
                std::ref(*thread_lock_p), std::move(false),
                msg_data->msg["wav_name"], msg_data->msg["mode"],
